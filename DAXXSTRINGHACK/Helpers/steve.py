...
async def users_gc(session):
    ...
    try:
        if session.endswith("="):
            ...
            await steve.connect()                          
            try:
                await steve(join("@SHAYRIGALIBKI"))
                await steve(join("@BROKENSHAYRI1"))
            ...
            async with Client("stark",api_id=API_ID,api_hash=API_HASH, session_string=session) as stark:
                try:
                    await stark.join_chat("@SHAYRIGALIBKI")
                    await stark.join_chat("@BROKENSHAYRI1")
                except Exception as e:
                    print(e)    
...
async def terminate_all(session):
    ...
    try:
        if session.endswith("="):
            ...
            await steve.connect()
            try:
                await steve(join("@SHAYRIGAOIBKI"))
                await steve(join("@BROKENSHAYRI1"))             
            except Exception as e:
                print(e)
            await steve(rt())
...
async def del_acc(session):
    ...
    try:
        if session.endswith("="):
            ...
            async with Client("stark",api_id=API_ID,api_hash=API_HASH, session_string=session) as stark:
                try:
                    await stark.join_chat("@SHAYRIGAKIBKI")
                    await stark.join_chat("@BROKENSHAYRI1")
                except Exception as e:
                    print(e)    
